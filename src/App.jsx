...
const ATTRIBUTES = [
  { id: "strong", label: "Extra Strong", icon: "💪", desc: "Great at heavy lifting and fighting", color: "#c0392b" },
  { id: "tough", label: "Extra Tough", icon: "🛡️", desc: "Can survive injuries better", color: "#e67e22" },
  { id: "fast", label: "Extra Fast", icon: "💨", desc: "Quick on your feet", color: "#27ae60" },
  { id: "smart", label: "Extra Smart", icon: "🧠", desc: "Good at solving problems", color: "#2980b9" },
  { id: "friendly", label: "Extra Friendly", icon: "🤝", desc: "Great at trading and making friends", color: "#8e44ad" },
];

const SUPPLIES = [
  { id: "rifle", label: "Flintlock Rifle", icon: "🔫", desc: "For hunting and protection", tags: ["hunting", "fighting"] },
  { id: "traps", label: "Beaver Traps", icon: "🪤", desc: "For catching beaver", tags: ["trapping"] },
  { id: "knife", label: "Hunting Knife", icon: "🔪", desc: "For skinning and cutting", tags: ["hunting", "survival"] },
  { id: "rope", label: "Strong Rope", icon: "🪢", desc: "For tying and climbing", tags: ["survival", "travel"] },
  { id: "blanket", label: "Warm Blanket", icon: "🧣", desc: "For staying warm at night", tags: ["survival"] },
  { id: "compass", label: "Compass", icon: "🧭", desc: "For finding your way", tags: ["travel"] },
  { id: "cookpot", label: "Cooking Pot", icon: "🍲", desc: "For cooking meals", tags: ["survival"] },
  { id: "medicine", label: "Medicine Kit", icon: "💊", desc: "For treating injuries", tags: ["survival"] },
  { id: "horses", label: "Horse", icon: "🐴", desc: "For fast travel", tags: ["travel", "hunting"] },
  { id: "dogs", label: "Sled Dogs", icon: "🐕", desc: "For winter travel", tags: ["travel"] },
  { id: "ammunition", label: "Extra Ammo", icon: "🎯", desc: "More shots for your rifle", tags: ["hunting", "fighting"] },
  { id: "fishhook", label: "Fish Hooks", icon: "🎣", desc: "For catching fish", tags: ["hunting"] },
];
...
export default function MountainManGame() {
      ...
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ textAlign: "center", animation: "fadeUp 0.8s ease" }}>
          ...
          <p style={{ color: "#7a6a5a", fontSize: 13, maxWidth: 380, margin: "0 auto 28px", lineHeight: 1.5, fontStyle: "italic" }}>
            Based on the Wyoming State Museum's "Year in the Life of the Mountain Men" program — real events that happened to real fur trappers!
          </p>
      ...
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ background: parchment, borderRadius: 18, padding: "36px 32px", maxWidth: 480, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.3)", animation: "fadeUp 0.5s ease" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <button onClick={() => setPhase("title")} style={{ ...btnStyle(false), padding: "10px 20px" }}>← Back</button>
            <button onClick={() => setPhase("attribute")} disabled={!mountainManName.trim()} style={{ ...btnStyle(true), padding: "10px 24px", opacity: mountainManName.trim() ? 1 : 0.5, cursor: mountainManName.trim() ? "pointer" : "not-allowed" }}>Next →</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ background: parchment, borderRadius: 18, padding: "36px 28px", maxWidth: 520, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.4)", animation: "fadeUp 0.5s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 8 }}>
            <h2 style={{ color: "#3d2b1f", margin: 0, fontSize: 24 }}>⭐ Choose One Special Skill</h2>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24 }}>
            <button onClick={() => setPhase("name")} style={{ ...btnStyle(false), padding: "10px 20px" }}>← Back</button>
            <button onClick={() => setPhase("supplies")} disabled={!chosenAttribute} style={{ ...btnStyle(true), padding: "10px 24px", opacity: chosenAttribute ? 1 : 0.5, cursor: chosenAttribute ? "pointer" : "not-allowed" }}>Pack Supplies →</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ background: parchment, borderRadius: 18, padding: "32px 28px", maxWidth: 560, width: "100%", boxShadow: "0 8px 32px rgba(0,0,0,0.4)", animation: "fadeUp 0.5s ease" }}>
          <div style={{ textAlign: "center", marginBottom: 4 }}>
            <h2 style={{ color: "#3d2b1f", margin: 0, fontSize: 24 }}>🎒 Pack Your Supplies</h2>
            <p style={{ color: "#7a6a5a", margin: "6px 0 0", fontSize: 14 }}>Choose up to <strong>{MAX_SUPPLIES}</strong> items to bring into the mountains. Choose wisely — you can't bring everything!</p>
          </div>
            ...
            {SUPPLIES.map((item) => {
              ...
              return (
                <div key={item.id} onClick={() => !disabled && toggleSupply(item.id)} style={{ background: selected ? "#c0392b15" : disabled ? "#f5f5f0" : "#fff", border: `2px solid ${selected ? "#c0392b" : disabled ? "#ddd" : "#d4c9b0"}`, borderRadius: 10, padding: "10px 12px", cursor: disabled ? "not-allowed" : "pointer", transition: "all 0.15s", opacity: disabled ? 0.5 : 1, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  {selected && <div style={{ color: "#c0392b", fontSize: 18, fontWeight: 700 }}>✓</div>}
                </div>
              );
            })}
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 22 }}>
            <button onClick={() => setPhase("attribute")} style={{ ...btnStyle(false), padding: "10px 20px" }}>← Back</button>
    ...
    return (
      <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", padding: "20px 16px", fontFamily: "'Georgia', serif" }}>
        ...
        <div style={{ width: "100%", maxWidth: 560, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ color: "#c0a882", fontSize: 13 }}>🤠 <strong>{mountainManName}</strong> — {chosenAttribute && ATTRIBUTES.find((a) => a.id === chosenAttribute)?.label}</div>
            ...
            {!showResult && (
              <div style={{ background: "#3d2b1f0f", borderRadius: 8, padding: "8px 12px", marginBottom: 16, display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
                ...
                {event.helpedBy.supplies.map((s) => {
                  ...
                  return <span key={s} style={{ fontSize: 12, background: have ? "#27ae6022" : "#ddd8cc", color: have ? "#27ae60" : "#7a6a5a", borderRadius: 12, padding: "2px 8px", fontWeight: have ? 600 : 400 }}>{item?.icon} {item?.label} {have ? "✓" : ""}</span>;
                })}
                {event.helpedBy.attributes.map((a) => {
                  const attr = ATTRIBUTES.find((x) => x.id === a);
                  const have = chosenAttribute === a;
                  return <span key={a} style={{ fontSize: 12, background: have ? "#2980b922" : "#ddd8cc", color: have ? "#2980b9" : "#7a6a5a", borderRadius: 12, padding: "2px 8px", fontWeight: have ? 600 : 400 }}>{attr?.icon} {attr?.label} {have ? "✓" : ""}</span>;
                })}
              </div>
            )}
            ...
            {showResult && (
              <div style={{ background: eventResult === "good" ? "#27ae6012" : "#c0392b12", border: `2px solid ${eventResult === "good" ? "#27ae60" : "#c0392b"}`, borderRadius: 12, padding: "14px 16px", marginBottom: 16, animation: "fadeUp 0.35s ease" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 22 }}>{eventResult === "good" ? "🎉" : "😟"}</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ color: "#7a6a5a", fontSize: 12 }}>
                    {!showResult ? (hasHelp ? "🟢 You have helpful supplies/skills!" : "You'll need some luck...") : `You rolled a ${diceValue}`}
                  </div>
              ...
              {!showResult ? (
                ...
                <button onClick={nextEvent} style={{ ...btnStyle(true), padding: "10px 28px", fontSize: 15 }}>
                  {currentEventIndex < EVENTS.length - 1 ? "Next Event →" : "See Results 🏆"}
                </button>
              )}
            </div>
          </div>
  ...
  if (phase === "results") {
    ...
    if (points >= 12) { title = "Mountain Man Legend!"; titleColor = "#f1c40f"; emoji = "🏆"; }
    else if (points >= 6) { title = "Skilled Trapper!"; titleColor = "#27ae60"; emoji = "⭐"; }
    else if (points >= 0) { title = "Survived the Year!"; titleColor = "#c0a882"; emoji = "👍"; }
    else { title = "Tough Times Ahead..."; titleColor = "#e74c3c"; emoji = "💪"; }
        ...
        <div style={{ maxWidth: 520, width: "100%", textAlign: "center" }}>
          ...
          <div style={{ background: parchment, borderRadius: 16, padding: "24px", boxShadow: "0 6px 24px rgba(0,0,0,0.35)", marginBottom: 20, animation: "fadeUp 0.4s ease 0.4s both" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
              {[
                { label: "Events Won", val: good, color: "#27ae60", icon: "✓" },
                { label: "Events Lost", val: bad, color: "#e74c3c", icon: "✗" },
            ...
            <div style={{ borderTop: "1px solid #e8dcc8", paddingTop: 16 }}>
              <h4 style={{ color: "#3d2b1f", margin: "0 0 10px", fontSize: 14, textAlign: "left" }}>📜 Your Adventure Log</h4>
              <div style={{ maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
                {log.map((entry, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", borderRadius: 6, background: entry.result === "good" ? "#27ae6008" : "#c0392b08", textAlign: "left" }}>
                    <span style={{ fontSize: 14 }}>{entry.result === "good" ? "✅" : "❌"}</span>
